...
class UIScene_NameInput : public UIScene
{
private:
	bool m_bDonePressed;
	bool m_bCompletionSent;
	int m_charLimit;
	NameInputCompleteFunc m_completeFunc;
	LPVOID m_completeFuncParam;

protected:
	UIControl_Label m_EnterTextLabel;
	UIControl_TextInput m_TextInput;
	...
	UI_BEGIN_MAP_ELEMENTS_AND_NAMES(UIScene)
		...
		UI_MAP_ELEMENT(m_TextInput, "KeyboardTextInput")
...
public:
	UIScene_NameInput(int iPad, void *initData, UILayer *parentLayer);
	~UIScene_NameInput();
...
private:
	bool processDirectKeyboardInput();
	void sanitiseCurrentInputText();
	void setInputText(const wstring &text, bool force = false);
	void backspaceInputText();
	void clearInputText();
	void completeInput(bool accepted);
	void handleDonePressed();

public:
	virtual EUIScene getSceneType() { return eUIScene_NameInput; }
